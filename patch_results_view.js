...
// Add Floating Chat component near the end of render
const chatUI = `

        {/* Floating AI Chat Iteration */}
        <div className="fixed bottom-8 left-1/2 -translate-x-3/4 lg:left-auto lg:-translate-x-0 lg:right-10 z-50 w-[400px] shadow-2xl">
          <motion.div
...
// Insert the UI just before returning the motion.div per tab.
data = data.replace(/(return \(\s*<motion\.div variants=\{containerState\} initial="hidden" animate="show" className="space-y-12 pb-20">)/g, chatUI + '$1');
data = data.replace(/(return \(\s*<motion\.div variants=\{containerState\} initial="hidden" animate="show" className="space-y-8 pb-20">)/g, chatUI + '$1');
data = data.replace(/(return \(\s*<motion\.div variants=\{containerState\} initial="hidden" animate="show" className="relative group pb-20 space-y-4">)/g, chatUI + '$1');

// need to import Sparkles, Send from lucide-react
