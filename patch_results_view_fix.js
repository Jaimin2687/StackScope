...
// 1. Remove the floating chat from the function body
const chatRegex = /\s*\{\/\* Floating AI Chat Iteration \*\/\}\s*<div className="fixed bottom-8[\s\S]*?<\/div>\s*(return \(\s*<motion\.div)/g;
data = data.replace(chatRegex, '$1');

// 2. We'll wrap the returns properly. Or even better, change how ResultsView returns.
// Since ResultsView returns early:
/*
...
// Let's create the chat UI string again
const chatUI = `
...
// we will just wrap the returned JSX in <>...</> and append chatUI before </>
data = data.replace(/(return \(\s*)(<motion\.div variants=\{containerState\} initial="hidden" animate="show" className="space-y-12 pb-20">)([\s\S]*?)(      <\/motion\.div>\s*\);)/, `$1<>\n$2$3$4\n${chatUI}\n</>\n);`);

data = data.replace(/(return \(\s*)(<motion\.div variants=\{containerState\} initial="hidden" animate="show" className="space-y-8 pb-20">)([\s\S]*?)(      <\/motion\.div>\s*\);)/, `$1<>\n$2$3$4\n${chatUI}\n</>\n);`);

data = data.replace(/(return \(\s*)(<motion\.div variants=\{containerState\} initial="hidden" animate="show" className="relative group pb-20 space-y-4">)([\s\S]*?)(      <\/motion\.div>\s*\);)/, `$1<>\n$2$3$4\n${chatUI}\n</>\n);`);
