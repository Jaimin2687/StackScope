...
const SCOPE_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    market_analysis: {
      type: SchemaType.OBJECT,
      properties: {
        viability_score: { type: SchemaType.NUMBER, description: "Market Viability Score strictly out of 100 (e.g. 85. NEVER use a 1-10 scale)" },
        reasoning: { type: SchemaType.STRING, description: "Brief reasoning for the score" },
        competitors: { type: SchemaType.ARRAY, items: { type: SchemaType.STRING }, description: "3 real-world competitors" }
      },
      required: ["viability_score", "reasoning", "competitors"]
    },
    estimates: {
      type: SchemaType.OBJECT,
      properties: {
  ...
  required: ["proposal", "sprint_timeline", "tech_stack", "sql_schema", "unit_economics", "detailed_architecture", "scalability_plan", "market_analysis", "estimates", "mermaid_diagram"],
} as Schema;
...
- UNIT ECONOMICS & SCALABILITY: Always compute estimated hosting costs in INR (₹) per month, break-even thresholds, detailed architecture including queues and cache, and step-by-step zero-to-million user scaling strategies. ALL unit economics and prices MUST be deeply aligned with the current Indian market and be highly feasible and realistic.\n- MARKET VIABILITY: You must provide a Market Analysis containing a viability score strictly out of 100 (e.g., 85. NEVER use a 1-10 scale), 2-3 sentences of reasoning, and identify 3 real-world competitors. Project estimates (base_cost_inr, optional_features) must reflect typical agency rates in India for a production-grade product, starting at realistic market values in INR.\n- ARCHITECTURE DIAGRAM: The Mermaid.js diagram MUST be exceptionally detailed, mapped to the true system architecture, showing all microservices, databases, queues, caches, and CDN layers.
This includes:
